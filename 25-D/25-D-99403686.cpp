    ...
    while(t--)
    {
        ...
        for(ll i=1; i<=n; i++)
        {
            for(ll j=1; j<=n; j++)
            {
                if(union_sets(i, j))
                {
                    aad.push_back(mp(i, j));
                }
            }
        }
        ll cnt = min({aad.size(), bad.size()});
        cout << cnt << endl;
        for(ll i=0; i<cnt; i++)
        {
            cout << bad[i].first << " " << bad[i].second << " " << aad[i].first << " " << aad[i].second << endl;
        }
    }
